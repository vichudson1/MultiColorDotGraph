...
class MultiColorDotGraph: UIView {
...
// MARK: -
private extension MultiColorDotGraph {
	...
	func plot(_ points: [ColoredGraphDot]) {
		lineLayer.path = nil
		dotLayer.path = nil
		data = nil
		...
		for i in points {
			let p = i.point.applying(chartTransform!)
			let rect = CGRect(x: p.x - radius, y: p.y - radius, width: radius * 2, height: radius * 2)
			let path = CGPath(ellipseIn: rect, transform: nil)
			let layer = CAShapeLayer()
			layer.fillColor = i.color.cgColor
			layer.path = path
			dotLayer.addSublayer(layer)
		}
	}
	
	func setAxisRange(forPoints points: [ColoredGraphDot]) {
		guard !points.isEmpty else { return }
		
		let xs = points.map() { $0.point.x }
		let ys = points.map() { $0.point.y }
